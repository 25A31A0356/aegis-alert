...
def run_server():
    ...
    print("=" * 80)
    print(" [AEGISALERT NATIONAL] SOVEREIGN OMNICHANNEL DISASTER MANAGEMENT SERVER")
    print(f" [*] Local URL:          http://localhost:{target_port}")
    print(f" [*] Serving Directory:  {DIRECTORY}")
    print(" [*] Omnichannel Endpoints:")
    print(f"     - GET  /api/status                 (System Health, Satellites & Towers)")
    print(f"     - GET  /api/satellites/live        (ISRO NavIC / INSAT Orbital Telemetry)")
    print(f"     - GET  /api/signal-pipeline/status (End-to-End Propagation Matrix)")
    print(f"     - POST /api/signal-pipeline/propagate (Satellite->NEOC->Towers->Devices)")
    print(f"     - GET  /api/calamities/live       (Multi-Calamity Ground Truth)")
    print(f"     - GET  /api/stream                (Server-Sent Events SSE Stream)")
    print(f"     - POST /api/alerts/broadcast      (Government Incident Command Dispatch)")
    print(f"     - POST /api/alerts/siren/trigger  (Geographical Siren Controller)")
    print(f"     - GET  /api/gov/export            (Unrestricted Open Data JSON/CSV)")
    print(f"     - POST /api/mobile/simulate-sms   (Citizen Smartphone Evac SMS)")
    print("=" * 80)
    print(f"Server actively listening on http://localhost:{target_port} ...")
    sys.stdout.flush()
